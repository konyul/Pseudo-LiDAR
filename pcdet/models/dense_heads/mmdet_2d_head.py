...
class MMDet2DHead(nn.Module):
    ...
    def get_loss(self, data_dict, tb_dict):
        img_metas = [{
            ...
            "img_shape": list(data_dict['images'][i].shape[1:3]) + [3],
            "pad_shape": list(data_dict['images'][i].shape[1:3]) + [3]}
            for i in range(len(data_dict['images']))]
        if self.use_3d_center:
            gt_boxes_2d = torch.cat([data_dict['gt_boxes2d'], data_dict['gt_centers_2d']], dim=-1)
        else:
            gt_boxes_2d = data_dict['gt_boxes2d']

        gt_boxes_2d = torch.unbind(gt_boxes_2d)
        # gt_boxes_3d = data_dict['gt_boxes_no3daug']
        gt_boxes_3d = data_dict['gt_boxes']
